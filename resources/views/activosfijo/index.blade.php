    ...
    <div class="card-body " style="overflow-x: scroll">
        <table class="table table-striped" id="activo" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $activo->id }}</td>
                    <td><img src="{{ asset( $activo->nombre) }}" width="50" height="50" />
                    <td>{{ $activo->nombre }}</td>
                    ...
                    <td>
                        <form action="{{ route('activosfijo.destroy', $activo) }}" method="post">
                            @csrf
                            @method('delete')
                            <a href="{{ route('activosfijo.show', $activo->id) }}" class="btn btn-warning btn-sm text-light rounded-pill"><i class="fas fa-eye"></i><a>
                                    <a href="{{ route('activosfijo.edit', $activo) }}" class="btn btn-primary btn-sm text-light rounded-pill"><i class="fas fa-edit"></i><a>
                                            @can('editar activo fijo')
                                            @endcan
                                            <button class="btn btn-danger btn-sm text-light rounded-pill" onclick="return confirm('¿ESTÁ SEGURO DE BORRAR?')" value="Borrar"><i class="fas fa-trash-alt"></i></button>
